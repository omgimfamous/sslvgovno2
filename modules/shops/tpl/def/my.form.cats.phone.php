<?php
    /**
     * Форма магазина: выбор категорий (phone)
     * @var $this Shops
     */
?>
<? if($step == 1) { # ШАГ 1 ?>
...
<div class="i-formpage__catselect__popup__sublist_mobile visible-phone">
    <div class="f-index__mobile__subcategories__title">
        ...
        <div class="subcat">
            <a href="#" class="backto ajax j-back" data="{prev:<?= $parent['pid'] ?>}">&laquo; <?= _t('shops','Вернуться назад') ?></a>
    ...
    <div class="f-index__mobile__subcategories__list">
        <ul>
             ...
             $i = sizeof($cats) - 1; $j = 0;
             foreach($cats as $v): ?>
             <li><a href="#" class="j-sub<? if($j++ == $i){ ?> last<? } if($v['active']) { ?> active<? } ?>" data="{id:<?= $v['id'] ?>,pid:<?= $parent['id'] ?>,subs:<?= $v['subs'] ?>,title:'<?= HTML::escape($v['t'], 'js') ?>'}"><?= $v['t'] ?><? if($v['subs'] > 0) { ?> <i class="icon-chevron-right pull-right"></i><? } ?></a></li>
             <? endforeach; ?>
        </ul>
    </div>
</div>
