 ...
 * @modified 6.sep.2015
 */
class UsersSocial extends Component
{
    ...
    /**
     * Получаем ключ провайдера по ID
     * @param integer $nProviderID ID провайдера
     * @return string '' если ID был указан некорректно
     */
    public function getProviderKeyByID($nProviderID)
    {
        $aConfig = config::file('social');
        if (!empty($aConfig['providers']) && is_array($aConfig['providers'])) {
            foreach ($aConfig['providers'] as $k => $v) {
                $nID = constant('self::PROVIDER_' . mb_strtoupper($k));
                if ($nID == $nProviderID) {
     ...
     * @return integer
     */
    public function auth($sProviderKey)
    {
     ...
     * @param integer $nUserID ID пользователя
     * @param boolean $bAdminView версию для отображения в профиле пользователя в админ. панели
     * @return array
     */
    public function getUserSocialAccountsData($nUserID, $bAdminView = false)
    {
        if (!empty($nUserID)) {
        ...
        if ($bAdminView) {
            foreach ($aData as $k=>&$v) {
                $v['provider_key'] = $this->getProviderKeyByID($k);
                $v['profile_data'] = func::unserialize($v['profile_data']);
                $v['profile_url']  = ( !empty($v['profile_data']['profileURL']) ?
                                             $v['profile_data']['profileURL'] : '');
            } unset($v);
        }
