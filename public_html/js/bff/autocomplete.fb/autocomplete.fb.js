  ...
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function createFCBK() {
        ...
        options.width = holder.outerWidth() + 5;
        if (options.attachto) {
      ...
      function addItem(title, value, preadded, locked, focusme, isNEW) {
        if (!maxItems()) {
          return false;
        }
        ...
        if(isNEW) { value = value.toString().replace(/(\"|\')/gi, ''); }
        ...
        if (!preadded) {
          ...
          addInput(focusme);
          var _item = $('<option value="'+( isNEW ? options.newel_prefix :'' )+xssDisplay(value, 1)+'" id="opt_'+id+'" class="selected" selected="selected">'+xssDisplay(title)+'</option>');
      ...
      function addInput(focusme) {
        ...
        input.keypress( function(event) {
          ...
          if(options.addoncomma && options.newel && (event.charCode || event.keyCode) == 44) {
              var etext = input.val(); //xssPrevent(input.val(), 1);
              if(etext!='') {
                addItem( etext, etext, 0, 0, 1, true);
              }
        ...
        maininput.keydown( function(event) {
          ...
          if ((event.keyCode == _key.enter || event.keyCode == _key.tab) && !checkFocusOn()) {
            if (options.newel) {
              ...
              if(value!='') {
                addItem(value, value, 0, 0, 1, true);
              }
              return _preventDefault(event);
            }
      ...
      function addTextItem(value) {
        if (options.newel && maxItems()) {
          ...
          var li = $('<li rel="0" fckb="1">').html(xssDisplay(value));
          feed.prepend(li);
          counter++;
        }
        return;
        ...
        if (typeof flag != "undefined") {
          for(i = 0; i < text.length; i++) {
            ...
            if ((_key.exclamation <= charcode && charcode <= _key.slash) ||
              ...
              text = text.replace(text[i], escape(text[i]));
            }
          }
        ...
        if (options.json_url && maxItems()) {
          if (options.cache && json_cache_object.get(etext)) {
            ...
            setTimeout( function() {
              ...
              $.post(options.json_url, {"tag": xssDisplay(etext)}, function(data) {
...
try{ bff.st.done('core/autocomplete.fb/autocomplete.fb.js'); }catch(e){}
